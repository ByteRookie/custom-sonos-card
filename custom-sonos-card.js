  ...
  determineActivePlayer(activePlayerId) {
    ...
    if (agsStatusEntity) {
      ...
      if (agsStatus && agsStatus !== "OFF") {
        let agsPlayerId = agsPrimaryEntity ? this.hass.states[agsPrimaryEntity]?.state : void 0;
        if (!agsPlayerId || agsPlayerId === "None") {
          agsPlayerId = agsPreferredEntity ? this.hass.states[agsPreferredEntity]?.state : void 0;
        }
        const agsGroup = agsPlayerId ? this.allGroups.find((group) => group.getMember(agsPlayerId) !== void 0) : void 0;
        if (agsGroup) {
          return agsGroup;
        }
      }
...
class Card extends i$4 {
  constructor() {
    ...
    this.agsStateListener = (event) => {
      if (event.data.entity_id === this.config.agsPrimarySpeakerSensor || event.data.entity_id === this.config.agsStatusSensor) {
...
class Grouping extends i$4 {
  ...
  render() {
    ...
    const agsSwitchState = this.config.agsSystemSwitch ? this.store.hass.states[this.config.agsSystemSwitch]?.state === "on" : false;
    const agsStatusState = this.config.agsStatusSensor ? this.store.hass.states[this.config.agsStatusSensor]?.state === "on" : false;
    ...
    return x`
      <div class="wrapper">
        <div class="predefined-groups">
          ...
          ${agsActive ? E : x`${this.renderJoinAllButton()}
              ${this.renderUnJoinAllButton()}${n2(this.store.predefinedGroups, () => this.renderPredefinedGroups())}`}
        </div>
        <div class="list">
          ${this.groupingItems.map((item) => {
      const roomSwitch = this.config.agsRoomSwitchPrefix ? `${this.config.agsRoomSwitchPrefix}${item.player.id.replace("media_player.", "")}` : "";
      const roomSwitchState = roomSwitch ? this.store.hass.states[roomSwitch]?.state === "on" : false;
      return x`
              <div
                class="item"
                modified=${!agsActive && item.isModified || E}
                disabled=${!agsActive && item.isDisabled || E}
              >
                ${agsActive ? x`<ha-switch
                      class="icon"
                      .checked=${roomSwitchState}
                      @change=${(ev) => this.toggleRoomSwitch(ev, roomSwitch)}
                    ></ha-switch>` : x`<ha-icon
                      class="icon"
        ...
        <ha-control-button-group
          class="buttons"
          hide=${agsActive || this.modifiedItems.length === 0 && !this.selectedPredefinedGroup || this.config.skipApplyButtonWhenGrouping || E}
        >
  ...
  renderAgsSwitch(checked) {
    return this.config.agsSystemSwitch ? x`<ha-switch .checked=${checked} @change=${this.toggleAgsSystem}></ha-switch>` : E;
  }
  async toggleAgsSystem(ev) {
    if (!this.config.agsSystemSwitch) {
      return;
    }
  ...
  async toggleRoomSwitch(ev, switchId) {
    if (!switchId) {
      return;
    }
    const checked = ev.target.checked;
...
__decorateClass$b([
  r$3()
], Grouping.prototype, "agsSwitchOn");
